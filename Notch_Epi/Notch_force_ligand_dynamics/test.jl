...
plot(sol, vars = [2,5,6,9,10], lw  = 1.5, 
	 xlabel = "Time", ylabel = "Concentration", title = "Switching Dynamics")
...
switch_amplitude = []
switch_frequency = []
@showprogress for freq =  exp.(-2:0.01:1.05), amplitude = 0:0.1:52
...
boundary_amplitude = []
for i in gp
	@show append!(boundary_amplitude,minimum(i.switch_amplitude))
end

critical_freq = [keys(gp)[i].switch_frequency for i = 1: gp.ngroups]
boundary_amplitude
plot!(plt, boundary_amplitude, critical_freq, label = "switching boundary", lw =3)
##
