...
# ╔═╡ 10ee6772-d62c-46a0-81db-6d75cdb08379
using ModelingToolkit, Plots, Latexify, PlutoUI, Images

# ╔═╡ 06c0567f-4ef1-45ad-8aa3-ace3171604ea
md"""
# Explaining the A-ω curve
"""

# ╔═╡ 2829ee2d-9f0b-40d8-812e-fa8eff9a4052
begin
...
# ╔═╡ fbe48325-5203-40a9-a51a-0633082d270c
md"""
### The ODE of the Notch-EMT system
"""

# ╔═╡ b0cac067-7f15-4c5d-9edc-be93c6e70bec
begin
...
# ╔═╡ 61ae7426-628c-4f65-9a2c-d57bb4b9c660
md"""
### 
The model for the Notch-EMT can be considered as an external module + a double-feedback module.\

- The external module consists the `MITF-NICD` competition as shown in below
- The bistable mainly comes from the `double-feedback` at histone level
"""

# ╔═╡ 3848eabb-e0a0-40ac-9bd5-7642d02c9b86
load("/Users/chentianchi/Desktop/Notch_EMT_plots/interaction.png")

# ╔═╡ 33056343-faff-4774-96a4-4b51b2164394
md"""
### Simulation results of A-ω curve
"""

# ╔═╡ d8326374-c95a-4b4e-81e0-ef05318bd019
load("/Users/chentianchi/Desktop/Notch_EMT_plots/A-w.png")

# ╔═╡ d7d8118f-1154-4206-bd70-827f7f2e9508
md"""
### The physics model for a bistable system in 1-D.

The curve describes how the external input signal **amplitude(A)** and **frequency(ω)** are affecting the states transitioning curve. The `orange curve` describe the `boundary` of when the Notch-EMT is swithing from one stable attractor to the other one. 

We observed that the curve is a nolinear and a parabolic shape. The following text will be focusing on a theoretical understanding of the `Notch-EMT` bistable system in the 1-D analogy.

1. The very often studied physics model that could generate 
"""

# ╔═╡ 393dd6be-d2da-4cea-b5a4-411691ab84d8
xs = -10:0.01:10

# ╔═╡ 834fb4e2-e1b5-4f54-a94d-138d807f33a9
begin
	md"""
	### Double well potential parameters(symmetric):
	c = $(@bind c Slider(-100.0:100.0, show_value=true, default = -10.)),
	β = $(@bind β Slider(-100.0:100.0, show_value=true, default = 20.)),
	"""
end

# ╔═╡ 7e1301a6-958c-4b20-91ca-213e3b192333
V(x) = 1/2*c*x^2 + 1/4*β*x^4

# ╔═╡ 20fa2c7c-71d7-4a87-9e7c-2d8fe3cd97c7
plot(xs, V.(xs), ylim =[-100,100])

# ╔═╡ c9ca429d-a075-49eb-a01e-f4941d306776
md"""
- In `V(x)` c is the natural frequency `(ω₀²)`. 
- `β` is the stiffness constant. 

Now the `Notch-EMT` 1d analogous model can be treated as a particle of unit mass moving in double-well potential  with friction. `d` is the damping constant.

$\ddot{x}+d \dot{x}+ cx+\beta x^{3}=A \cos \omega t$ 
in a double-well potential:

$V(x)=\frac{1}{2} \omega_{0}^{2} x^{2}+\frac{1}{4} \beta x^{4}$
subjected to a viscous drag force of strength d and driven by an external periodic force of **frequency ω** and **amplitude A**.

__Now the simplified dynamics can be modelled by the damped and forced Dufﬁng oscillator equation__.\
Assume the periodic solution of:

$x(t)=a(t) \cos \omega t+b(t) \sin \omega t$
where a(t) and b(t) are slowly varying functions of time t

"""

# ╔═╡ 0ab7f8d7-2368-41f0-9047-c437fd7871bd
# using Symbolics

# ╔═╡ 618b5411-9d62-4f52-911f-bc5196bb6ff3
@variables a(t) b(t)

# ╔═╡ eb188028-b50a-411b-ad22-489d6e77ce3e
# The general solution of equation
x = a*cos(ω*t) + b*sin(ω*t)

# ╔═╡ 54e583a5-abd8-44f1-824b-b234412bd3a5
expand(x^3)

# ╔═╡ af5e0436-785b-49b7-b9c7-f771680b54a9
md"""
Substitution of the solution
$x^{3} \approx \frac{3}{4}\left(a^{2}+b^{2}\right)(a \cos \omega t+b \sin \omega t)$
"""

# ╔═╡ 242ca8fe-53dc-4a0f-9f05-f8b12f00d0e5
expand((a^2 + b^2)*(a*cos(ω*t) + b*sin(ω*t)))

# ╔═╡ 2ffd237d-4e1a-4e88-acfc-8cf7e63a82c9
eq_expression = expand_derivatives(D(D(x)) - d*D(x) + c*x + β*x^3)

# ╔═╡ c0944c36-ccfe-43cc-82e2-05f87147f3f6
@variables F0, ϕ

# ╔═╡ 1e53d491-b66d-4cf3-9a6b-c5679eee1a99

expand(c/2*(F0*cos(ϕ)*cos(ω*t) + F0*sin(ϕ)*sin(ω*t))^2 + β/4*(F0*cos(ϕ)*cos(ω*t) + F0*sin(ϕ)*sin(ω*t))^4)

# ╔═╡ 9a28c411-5bb8-45c4-b71c-0f4d07486b21
md"""
neglecting $\ddot a$ , $\ddot b$, d$\ddot a$ and d$\ddot b$ as they are assumed to be small and then equating the coefﬁcients of sin(ωt) and cos(ωt) separately to zero give

$\dot{a}=-\frac{b}{2 \omega}\left[\omega^{2}-\omega_{0}^{2}-\frac{3}{4} \beta\left(a^{2}+b^{2}\right)\right]-\frac{1}{2} d a$
$\dot{b}=\frac{a}{2 \omega}\left[\omega^{2}-\omega_{0}^{2}-\frac{3}{4} \beta\left(a^{2}+b^{2}\right)\right]-\frac{1}{2} d b+\frac{A}{2 \omega}$

Introducing the transformation
$a(t)=F(t) \cos \phi(t), \quad b(t)=F(t) \sin \phi(t)$\
with $F^{2}=a^{2}+b^{2}$

The two equations above become:

$\begin{aligned} \dot{F} &=-\frac{1}{2} d A+\frac{A}{2 \omega} \sin \phi \\ F \dot{\phi} &=\frac{F}{2 \omega}\left[\omega^{2}-\omega_{0}^{2}-\frac{3}{4} \beta A^{2}\right]+\frac{A}{2 \omega} \cos \phi \end{aligned}$

In the long time limit, after a transient motion, the solution $x(t)=a(t) \cos \omega t+b(t) \sin \omega t$ becomes periodic with period $2 \pi / \omega$ provided $a(t)$ and $b(t)$ become constants as $t \rightarrow \infty$. The limiting values of $a$ and $b$ are related to the equilibrium points of the above equations and are obtained by setting $\dot{F}=\dot{\phi}=0, F(t)=F^{*}, \phi(t)=\phi^{*}$ and dropping ' $*$ ' in $A$ and $\phi$ for simplicity. The result is

$\begin{aligned} d F \omega &=A \sin \phi, \\ F\left[\omega_{0}^{2}-\omega^{2}+\frac{3}{4} \beta F^{2}\right] &=A \cos \phi . \end{aligned}$
squared each equations and sum ==>

$F^{2}\left[\left(\omega_{0}^{2}-\omega^{2}+\frac{3}{4} \beta F^{2}\right)^{2}+d^{2} \omega^{2}\right]-A^{2}=0$
"""

# ╔═╡ e95e595d-d11c-44ed-94f6-02122de61908
md"""
In our Notch-EMT case, we would like to find given a fixed potential difference between the saddle point and one of the minimum, what is the A-ω relation. 
Therefore, we set the $F^{2}$ term to be `constant` because it porpotional to the potential difference.
"""

# ╔═╡ 852900c0-b460-4018-b7e8-4d8fcda6be53
begin
	md"""
	### Double well potential parameters(symmetric):
	F = $(@bind F Slider(0.0:0.01:10.0, show_value=true, default = 1.)),
	ω0 = $(@bind ω0 Slider(0.0:0.01:10.0, show_value=true, default = 1.)),
	d0 = $(@bind d0 Slider(0.0:0.01:10.0, show_value=true, default = 1.)),
	"""
end

# ╔═╡ 691901f7-7f69-4339-a0d8-84d6e196df8b
A_eq(ω) = sqrt(F^2*((-c - ω^2 + 3/4*β*F^2)^2 + d0^2*ω^2))

# ╔═╡ c8d63f72-a04b-4aea-8c5f-ef646d86a555
ωs = -10:0.01:10

# ╔═╡ 426c5360-144e-4d96-b302-759780201746
A_eq.(ωs)

# ╔═╡ db7f1d1d-7e44-4b21-86e5-26ff0aa06785
plot( A_eq.(ωs),ωs, xlabel = "Switch_amplitude(A)",ylabel= "Switch_frequency(ω)")#,ylim = [0,10], xlim = [0,80]

# ╔═╡ e4d33cac-d8da-43d4-9972-65c0a1c60655
md"""
The phase of the external signal is a function of intput frequency ω parameterized by ω0, β, F

$\phi=\tan ^{-1}\left[\frac{d \omega}{\omega_{0}^{2}-\omega^{2}+\frac{3}{4} \beta F^{2}}\right]$
"""

# ╔═╡ 46ca2f87-f748-4297-8360-ba1ad272452d
Φ(ω) = atan(d0*ω/ω0^2 - ω^2 + 3/4*β*F^2)

# ╔═╡ 6a74c0b9-6bf1-4681-9205-098e9e28af74
plot(Φ.(ωs), ωs, ylim= [0,10], xlabel = "Switch_phase(Φ)",ylabel= "Switch_frequency(ω)")

# ╔═╡ df076e72-89b7-4335-bd10-222d1f375e86
md"""
### Addding $x^{3}$ term to make the potential surface asymmetrical
"""
