...
function df4d_fix_phase_freq_vs_ST(df4d_phase_0; ΔT = 100, save = false)
    @show db_idx
    plt = @df df4d_phase_0 plot(
        :freq,
        :stime,
        group =  :amp,
        palette = :RdYlBu_6,
    ...
    xlabel!(plt, "Switching Frequency")
    ylabel!(plt, "Switching Time")
    if save
...
function df4d_fix_amp_freq_vs_ST(df4d_amp_select; ΔT = 100, save = false)
    @show db_idx
    plt = @df df4d_amp_select plot(
        :freq,
        :stime,
        group = :phase,
        palette = :RdYlBu_7,
